...
class GPUModelRunner(
    ...
    def _sync_num_accepted_tokens(
        ...
        """Synchronize accepted counts into the current InputBatch rows.

        The asynchronous D2H copy always targets the runner-owned
        ``num_accepted_tokens`` buffer.  Only after its event has completed do
        we remap that previous-iteration snapshot through ``prev_positions``
        and write it into the mutable InputBatch.  This prevents condense or
        request-row changes from racing an in-flight DMA write.
        """
        if self.use_async_scheduling:
    ...
    def synchronize_input_prep(self):
        # Input-batch row moves and block-table updates must not begin while
        # the previous speculative-decode postprocess is still consuming the
        # same persistent metadata and publishing accepted-token counts.
        # This event is recorded after that GPU postprocess is enqueued.
